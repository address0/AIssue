      ...
      <div className="bg-white w-96 rounded-lg p-6 shadow-lg relative">
        ...
        <div className="mt-4 space-y-2 max-h-64 overflow-y-auto">
          {tasks.map((task, index) => (
            <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-md shadow-sm border border-gray-200">
              <div>
                <h3 className="text-sm font-semibold text-orange-600">{task.title}</h3>
                <p className="text-xs text-gray-500">Start: {task.start.toLocaleString()}</p>
                <p className="text-xs text-gray-500">End: {task.end.toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>
